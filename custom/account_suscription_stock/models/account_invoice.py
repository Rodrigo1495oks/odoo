...
class AccountMove(models.Model):
    ...
    def _stock_account_prepare_anglo_saxon_in_integration_lines_vals(self):
        ...
        for move in self:
            ...
            for line in move.invoice_line_ids:
                ...
                valuation_stock_moves = self.env['stock.move'].search([
                    ('suscription_line_id', '=', line.suscription_product_line_id.id),
                    ('state', '=', 'done'),
                    ('product_qty', '!=', 0.0),
                ]) if line.suscription_product_line_id else self.env['stock.move']

                if line.product_id.cost_method != 'standard' and line.suscription_product_line_id:
    ...
    def _post(self, soft=True):
        ...
        self.env['account.move.line'].create(self._stock_account_prepare_anglo_saxon_in_integration_lines_vals())
        return super()._post(soft)
    ...
    def _stock_account_get_last_step_stock_moves(self):
        ...
        for invoice in self.filtered(lambda x: x.move_type == 'in_invoice'):
            rslt += invoice.mapped('invoice_line_ids.suscription_product_line_id.move_ids').filtered(
