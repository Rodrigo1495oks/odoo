...
class AssemblyMeeting(models.Model):
    ...
    def default_get(self, fields):
        ...
        defaults = super(AssemblyMeeting, self).default_get(fields)
    ...
    shareholders = fields.Many2many(string='Accionistas Presentes', comodel_name='account.shareholder',
    ...
    def _compute_stop(self):
        ...
        for event in self:
            ...
            event.stop = event.start + \
                timedelta(minutes=round((event.duration or 1.0) * 60))
            if event.allday:
    ...
    def _compute_recurrence(self):
        ...
        for event in self:
            if event.recurrency:
                ...
                event.update({**false_values, **defaults, **
                             event_values, **rrule_values})
            else:
    ...
    def _get_ics_file(self):
        ...
        for meeting in self:
            ...
            event.add('dtstart').value = ics_datetime(
                meeting.start, meeting.allday)
            event.add('dtend').value = ics_datetime(
                meeting.stop, meeting.allday)
    ...
    def _attendees_values(self, partner_commands):
        ...
        for command in partner_commands:
            ...
            if op in (2, 3):  # Remove partner
                ...
                removed_partner_ids += set(self.attendee_ids.mapped(
                    'partner_id').ids) - set(command[2])
                added_partner_ids += set(command[2]) - \
                    set(self.attendee_ids.mapped('partner_id').ids)
            elif op == 4:
                added_partner_ids += [command[1]] if command[1] not in self.attendee_ids.mapped(
                    'partner_id').ids else []
        ...
        attendee_commands += [[2, attendee.id]
                              for attendee in attendees_to_unlink]  # Removes and delete
    ...
    def get_interval(self, interval, tz=None):
        ...
        if interval == 'day':
            ...
            result = babel.dates.format_date(
                date=date, format='MMMM y', locale=get_lang(self.env).code)

        elif interval == 'dayname':
            # Localized day name
            result = babel.dates.format_date(
                date=date, format='EEEE', locale=get_lang(self.env).code)
    ...
    def _apply_recurrence_values(self, values, future=True):
        ...
        for event in self:
            if not event.recurrence_id:
                recurrence_vals += [dict(values, base_event_id=event.id,
    ...
    def _split_recurrence(self, time_values):
        ...
        previous_week_day_field = weekday_to_field(
            self._get_start_date().weekday())
    ...
    def write(self, values):
        ...
        recurrence_values = {field: values.pop(
            field) for field in self._get_recurrent_fields() if field in values}
        if update_recurrence:
            if break_recurrence:
                ...
                time_values = {field: values.pop(
                    field) for field in self.env['calendar.event']._get_time_fields() if field in values}
                if not update_start and (time_values or recurrence_values):
                    raise UserError(
                        _("Updating All Events is not allowed when dates or time is modified. You can only update one particular event and following events."))
        ...
        if 'start' in values:
            ...
            if start_date and start_date >= fields.Datetime.now():
                (current_attendees & previous_attendees)._send_mail_to_attendees(
                    'calendar.calendar_template_meeting_changedate', ignore_recurrence=not update_recurrence)
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            ...
            if not values.get('activity_ids'):
                ...
                res_model_id = values.get(
                    'res_model_id', defaults.get('res_model_id'))
        ...
        for event, vals in zip(recurring_events, recurring_vals):
            recurrence_values = {field: vals.pop(
                field) for field in recurrence_fields if field in vals}
            if vals.get('recurrency'):
                detached_events = event._apply_recurrence_values(
                    recurrence_values)
                detached_events.active = False

        events.filtered(lambda event: event.start > fields.Datetime.now(
        )).attendee_ids._send_mail_to_attendees('calendar.calendar_template_meeting_invitation')
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        def split_privacy(events):
            ...
            """
            private = [event for event in events if event.get(
                'privacy') == 'private']
            public = [event for event in events if event.get(
                'privacy') != 'private']
        ...
        def my_events(events):
            ...
            """
            my = [event for event in events if event.get(
                'user_id') and event.get('user_id')[0] == self.env.uid]
            others = [event for event in events if not event.get(
                'user_id') or event.get('user_id')[0] != self.env.uid]
            return my, others
        ...
        def obfuscated(events):
            ...
            return [{
                field: hide(
                    field, value) if field not in public_fields else value
                for field, value in event.items()
            } for event in events]

        events = super().read(fields=fields +
                              ['privacy', 'user_id'], load=load)
    ...
    def name_get(self):
        ...
        shown = self - hidden
        shown_names = super(AssemblyMeeting, shown).name_get()
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        grouped_fields = set(group_field.split(
            ':')[0] for group_field in groupby)
        ...
        return super(AssemblyMeeting, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
        ...
        for event in self:
            if event.activity_ids:
                ...
                if 'description' in fields:
                    activity_values['note'] = event.description and tools.plaintext2html(
                        event.description)
                if 'start' in fields:
...
class AssemblyMeetingTopic(models.Model):
    ...
    short_name = fields.Char(string='Titulo')
