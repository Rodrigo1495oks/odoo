...
class SuscriptionMove(models.Model):
    _name = "suscription.move"
    _inherits = {"account.move", 'account_move_id'}
    _description = "Suscription Entry"
    ...
    def _compute_invoice_payment_term_id(self):
        for move in self:
            ...
            elif move.is_suscription_document(include_receipts=True) and move.partner_id.property_supplier_payment_term_id:
                move.invoice_payment_term_id = move.partner_id.property_supplier_payment_term_id
            elif move.is_integration_document(include_receipts=True) and move.partner_id.property_supplier_payment_term_id:
                move.invoice_payment_term_id = move.partner_id.property_supplier_payment_term_id
            else:
    ...
    def _compute_invoice_filter_type_domain(self):
        for move in self:
            ...
            elif move.is_suscription_document(include_receipts=True):
                move.invoice_filter_type_domain = 'suscription'
            elif move.is_integration_document(include_receipts=True):
                move.invoice_filter_type_domain = 'integration'
            else:
    ...
    def _check_journal_move_type(self):
        for move in self:
            ...
            if move.is_suscription_document(include_receipts=True) and move.journal_id.type != 'suscription':
                raise ValidationError(_("Cannot create a sale document in a non sale journal"))
            if move.is_integration_document(include_receipts=True) and move.journal_id.type != 'integration':
                raise ValidationError(_("Cannot create a sale document in a non sale journal"))
    ...
    def _post(self, soft=True):
        ...
        for invoice in self.filtered(lambda move: move.is_invoice(include_receipts=True)):
            ...
            if not invoice.partner_id:
                ...
                elif invoice.is_suscription_document():
                    raise UserError(_("The field 'Shareholder' is required, please complete it to validate the Vendor Bill."))
                elif invoice.is_integration_document():
                    raise UserError(_("The field 'Shareholder' is required, please complete it to validate the Vendor Bill."))
            ...
            if not invoice.invoice_date:
                ...
                elif invoice.is_suscription_document(include_receipts=True):
                    raise UserError(_("The Bill/Refund date is required to validate this document."))
                elif invoice.is_integration_document(include_receipts=True):
                    raise UserError(_("The Bill/Refund date is required to validate this document."))
    ...
    def _compute_invoice_payment_term_id(self):
        for move in self:
            if move.is_sale_document(include_receipts=True) and move.partner_id.property_payment_term_id:
                ...
                move.invoice_payment_term_id = move.partner_id.property_payment_term_id
            elif move.is_integration_document(include_receipts=True) and move.partner_id.property_payment_term_id:
                move.invoice_payment_term_id = move.partner_id.property_payment_term_id
            else:
                move.invoice_payment_term_id = False
        # === Partner fields === #
