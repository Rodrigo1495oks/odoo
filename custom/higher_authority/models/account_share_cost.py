...
class AccountShareCostOrder(models.Model):
    _name = 'account.share.cost'
    _description = 'Objeto Costos de Emisión'
    _rec_name = 'partner_ref'
    ...
    def _get_integrated(self):
        ...
        for order in self:
            ...
            if any(
                ...
                order.invoice_status = 'to integrate'
            elif (
                all(
                ...
                order.invoice_status = 'integrated'
            else:
                order.invoice_status = 'no'
    ...
    # métodos exclusivos de integración

    @api.depends('product_id', 'cash_id')
    def _compute_qty_to_integrate(self):
        for order in self:
            for line in order.order_line:
                order.qty_to_integrate+= line.price_total
            order.qty_to_integrate+=order.cash_subscription

    @api.depends('amount_total','cash_subscription')
    def _compute_amount_total_order(self):
        for order in self:
            order.amount_total_order =order.amount_total+order.cash_subscription

    @api.depends('integration_order')
    def _compute_qty_integrated(self):
        for order in self:
            for integration in order.order_line:
                order.qty_integrated+=integration.price_total
    ...
    def _compute_qty_pending(self):
        for order in self:
            order.qty_pending=order.qty_integrated-order.amount_total_order
    @api.depends('qty_integrated')
    def _compute_order_is_pending(self):
            for order in self:
                if order.qty_pending>0.0: order.pending=True
                else: order.pending=False
    
    # campos de integracion

    int_date=fields.Date(string='Fecha de Costo', readonly=True)
    ...
    # pasar este valor desde la orden de subscription_orde
    

    amount_total_order = fields.Monetary(
        string='Total', store=True, readonly=True, compute='_compute_amount_total_order')
    # campos relacionales
    ...
    partner_id = fields.Many2one('res.partner', string='Accionista', required=True, states=READONLY_STATES, change_default=True, tracking=True, domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]", help="You can find a vendor by its Name, TIN, Email or Internal Reference.", related='partner_id.id')
    ...
    order_line = fields.One2many('account.share.cost.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True)
    notes = fields.Html('Terms and Conditions')

    invoice_count = fields.Integer(compute="_compute_invoice", string='Integraciones', copy=False, default=0, store=True)
    invoice_ids = fields.Many2many('account.move', compute="_compute_invoice", string='Integraciones', copy=False, store=True)
    invoice_status = fields.Selection([
        ('no', 'Nothing to Invoice'),
        ('to integrate', 'Facturas Pendientes'),
        ('integrated', 'Totalmente Facturado'),
    ], string='Billing Status', compute='_get_integrated', store=True, readonly=True, copy=False, default='no')
    ...
    # campos que integran el stock
     # campos para administrar recepciones (traidos de AccountShareCostOrderStock)
    incoterm_id = fields.Many2one('account.incoterms', 'Incoterm', states={'done': [('readonly', True)]}, help="International Commercial Terms are a series of predefined commercial terms used in international transactions.")
    ...
    def _compute_access_url(self):
        super(AccountShareCostOrder, self)._compute_access_url()
        for order in self:
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if vals.get('name', 'New') == 'New':
                ...
                vals['name'] = self_comp.env['ir.sequence'].next_by_code('account.share.cost', sequence_date=seq_date) or '/'
            vals, partner_vals = self._write_partner_values(vals)
            partner_vals_list.append(partner_vals)
            orders |= super(AccountShareCostOrder, self_comp).create(vals)
    ...
    def copy(self, default=None):
        ...
        new_po = super(AccountShareCostOrder, self).copy(default=default)
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        """
        result = super(AccountShareCostOrder, self).onchange(values, field_name, field_onchange)
    ...
    def message_post(self, **kwargs):
        ...
        return super(AccountShareCostOrder, self.with_context(mail_post_autofollow=self.env.context.get('mail_post_autofollow', True))).message_post(**kwargs)

    def _notify_get_recipients_groups(self, msg_vals=None):
        """ Tweak 'view document' button for portal customers, calling directly
        routes for confirm specific to PO model. """
        groups = super(AccountShareCostOrder, self)._notify_get_recipients_groups(msg_vals=msg_vals)
        if not self:
            return groups
    ...
    def _track_subtype(self, init_values):
        ...
        return super(AccountShareCostOrder, self)._track_subtype(init_values)
    ...
    def _approval_allowed(self):
        ...
        return (
            ...
            or self.user_has_groups('higher_authority.group_share_cost_manager'))
...
class AccountShareCostOrderLine(models.Model):
    _name = 'account.share.cost.line'
    _inherit = 'analytic.mixin'
    _description = 'Account Share Cost Order Line'
    ...
    # metodos de stock
    # campos de purchase_Stock
    qty_received_method = fields.Selection(selection_add=[('stock_moves', 'Stock Moves')])
                                            # share_cost_line_id
    move_ids = fields.One2many('stock.move', 'share_cost_line_id', string='Reservation', readonly=True, copy=False)
    orderpoint_id = fields.Many2one('stock.warehouse.orderpoint', 'Orderpoint', copy=False, index='btree_not_null')                     # share_cost_line_id
    move_dest_ids = fields.One2many('stock.move', 'created_share_cost_line_id', 'Downstream Moves')
    ...
    def write(self, values):
        ...
        return super(AccountShareCostOrderLine, self).write(values)
    ...
    def action_share_cost_history(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("purchase.action_share_cost_history")
    ...
    # metodos para stock
    def _compute_qty_received_method(self):
        super(AccountShareCostOrderLine, self)._compute_qty_received_method()
        for line in self.filtered(lambda l: not l.display_type):
    ...
    def _compute_qty_received(self):
        ...
        super(AccountShareCostOrderLine, self - from_stock_lines)._compute_qty_received()
        for line in self:
    ...
    @api.model_create_multi
    def create(self, vals_list):
        lines = super(AccountShareCostOrderLine, self).create(vals_list)
    ...
    def write(self, values):
        ...
        result = super(AccountShareCostOrderLine, self).write(values)
    ...
    def _create_or_update_picking(self):
        for line in self:
            if line.product_id and line.product_id.type in ('product', 'consu'):
                ...
                if float_compare(line.product_qty, line.qty_integrated, line.product_uom.rounding) == -1:
                    # If the quantity is now below the integrated quantity, create an activity on the vendor bill
    ...
    def _create_stock_moves(self, picking):
        ...
        for line in self.filtered(lambda l: not l.display_type):
            for val in line._prepare_stock_moves(picking):
                values.append(val)
            line.move_dest_ids.created_share_cost_line_id = False
