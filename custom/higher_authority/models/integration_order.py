...
class IntegrationOrder(models.Model):
    ...
    qty_pending=fields.Monetary(string='Cantidad pendiente de integrar',currency_field='company_currency_id',compute='_compute_qty_pending', help='Cantidad que no se ha subscripto')
    ...
    order_line = fields.One2many('integration.order.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True)
    ...
    invoice_status = fields.Selection([
        ...
        ('to integrate', 'Integraciones Pendientes'),
        ('integrated', 'Totalmente Integrado'),
...
class IntegrationOrderLine(models.Model):
    ...
    order_id = fields.Many2one('integration.order', string='Order Reference', index=True, required=True, ondelete='cascade')

    company_id = fields.Many2one('res.company', related='order_id.company_id', string='Company', store=True, readonly=True)
    state = fields.Selection(related='order_id.state', store=True)

    invoice_lines = fields.One2many('account.move.line', 'integration_line_id', string="Bill Lines", readonly=True, copy=False)
    ...
    # metodos para stock
    def _compute_qty_received_method(self):
        super(IntegrationOrderLine, self)._compute_qty_received_method()
        for line in self.filtered(lambda l: not l.display_type):
    ...
    def _compute_qty_received(self):
        ...
        super(IntegrationOrderLine, self - from_stock_lines)._compute_qty_received()
        for line in self:
    ...
    @api.model_create_multi
    def create(self, vals_list):
        lines = super(IntegrationOrderLine, self).create(vals_list)
    ...
    def write(self, values):
        ...
        result = super(IntegrationOrderLine, self).write(values)
...
class IntegrationOrderLineCredit(models.Model):
    _name = 'integration.order.line.credit'
